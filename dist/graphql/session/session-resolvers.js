...
const mapSessionToGraphQL = (session) => {
    return {
        ...
        clientsIds: session.clients_ids?.map((id) => id.toString()) || [],
    ...
    Mutation: {
        createSession: async (_, { input }, context) => {
            ...
            const session = new Session({
                ...
                clients_ids: input.clientsIds.map((id) => new mongoose.Types.ObjectId(id)),
                name: input.name,
        ...
        clients: async (parent) => {
            if (parent.clientsIds && parent.clientsIds.length > 0) {
                const clients = await User.find({
                    _id: { $in: parent.clientsIds.map((id) => new mongoose.Types.ObjectId(id)) },
                })
                    .select('firstName lastName email')
                    .lean();
