...
export default {
    CoachRating: {
        ...
        sessionLog: async (parent) => {
            if (parent.sessionLog)
                return parent.sessionLog;
    ...
    Query: {
        getCoachRatings: async (_, { coachId }, context) => {
        ...
        getCoachRatingBySessionLog: async (_, { sessionLogId }, context) => {
            const userId = context.auth.user?.id;
            if (!userId) {
                throw new Error('Unauthorized: Please log in');
            }
    ...
    Mutation: {
        createCoachRating: async (_, { input }, context) => {
            const userId = context.auth.user?.id;
            const userRole = context.auth.user?.role;
            ...
            const sessionLog = await SessionLog.findById(new mongoose.Types.ObjectId(input.sessionLogId)).lean();
            if (!sessionLog) {
        ...
        updateCoachRating: async (_, { id, rating, comment }, context) => {
            const userId = context.auth.user?.id;
            const userRole = context.auth.user?.role;
            ...
            const existingRating = await CoachRating.findById(new mongoose.Types.ObjectId(id)).lean();
            if (!existingRating) {
            ...
            const updatedRating = await CoachRating.findByIdAndUpdate(new mongoose.Types.ObjectId(id), updateData, { new: true })
                .populate('coach_id', 'firstName lastName email')
                .populate('client_id', 'firstName lastName email')
                .populate('sessionLog_id')
                .lean();
            // Update coach's average rating
            if (rating !== undefined) {
                await updateCoachAverageRating(existingRating.coach_id?.toString() || '');
            }
            return mapCoachRatingToGraphQL(updatedRating);
        },
        deleteCoachRating: async (_, { id }, context) => {
            const userId = context.auth.user?.id;
            const userRole = context.auth.user?.role;
            ...
            const rating = await CoachRating.findById(new mongoose.Types.ObjectId(id)).lean();
            if (!rating) {
                throw new Error('Rating not found');
            }
            // Verify the rating belongs to the current user
