...
export default {
    Query: {
        getGoals: async (_, { clientId, status }, context) => {
        ...
        getAllClientGoals: async (_, { coachId, status }, context) => {
            const userId = context.auth.user?.id;
            const userRole = context.auth.user?.role;
            ...
            if (userIdString !== coachIdString && userRole !== 'admin') {
                throw new Error('Unauthorized: You can only view goals for your assigned clients');
            }
        ...
        getWeightProgressChart: async (_, { clientId, goalId }, context) => {
            const userId = context.auth.user?.id;
            const userRole = context.auth.user?.role;
            if (userId !== clientId && userRole !== 'admin') {
    ...
    Mutation: {
        ...
        updateGoal: async (_, { id, input }, context) => {
            const userId = context.auth.user?.id;
            const userRole = context.auth.user?.role;
            ...
            const isCoach = goal.coach_id && goal.coach_id.toString() === userId;
            const isAdmin = userRole === 'admin';
            if (!isClient && !isCoach && !isAdmin) {
        ...
        assignCoachToGoal: async (_, { goalId }, context) => {
            const userId = context.auth.user?.id;
            const userRole = context.auth.user?.role;
            if (userRole !== 'coach' && userRole !== 'admin') {
                throw new Error('Unauthorized: Only coaches or admins can assign coaches to goals');
            }
            if (!userId) {
                throw new Error('Unauthorized: Please log in');
            }
            ...
            const coachClientIds = (coach.coachDetails?.clients_ids || []).map((id) => {
                if (id instanceof mongoose.Types.ObjectId) {
                    return id.toString();
                }
                return String(id);
            ...
            if (!isClientOfCoach && userRole !== 'admin') {
                throw new Error('Unauthorized: You can only assign yourself to goals of your clients');
            }
            // Update goal with coach assignment
            // Use the string version of goalId to ensure it's valid
            const updatedGoal = await Goal.findByIdAndUpdate(goalIdString, { coach_id: new mongoose.Types.ObjectId(coachIdString) }, { new: true })
                .populate('client_id', 'firstName lastName email')
                .populate('coach_id', 'firstName lastName email')
                .lean();
            if (!updatedGoal) {
