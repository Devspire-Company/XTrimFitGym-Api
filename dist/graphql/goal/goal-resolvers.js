        ...
        getWeightProgressChart: async (_, { clientId, goalId }, context) => {
            ...
            if (userId !== clientId && userRole !== 'admin') {
                ...
                if (userRole === 'coach') {
                    ...
                    const isClientOfCoach = coach?.coachDetails?.clients_ids?.some((id) => id.toString() === clientId);
                    if (!isClientOfCoach) {
                        throw new Error('Unauthorized: You can only view progress of your clients');
                    }
                }
                else {
