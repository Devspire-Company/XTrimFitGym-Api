...
export default {
	Query: {
		...
		getRevenueSummary: async (
			...
			let totalRevenue = 0;
			let revenueByMembership: Array<{ membershipId: string; membershipName: string; revenue: number; count: number }> = [];

			if (todayAnalytics) {
				totalRevenue = todayAnalytics.totalRevenue;
				revenueByMembership = todayAnalytics.revenueByMembership.map((r: any) => ({
					membershipId: r.membershipId.toString(),
					membershipName: r.membershipName,
					revenue: r.revenue,
					count: r.count,
			...
			let periodRevenue: Array<{ period: string; revenue: number; count: number }> = [];
