...
- **ATTENDANCE_id** `VARCHAR(50)` **PRIMARY KEY** (not INT; IVMS can send string event IDs).
- **eventTime** `DATETIME` **NOT NULL** (single canonical time; no numeric-only type).
...
- **UNIQUE** only on ATTENDANCE_id (PK implies uniqueness).
...
- **INSERT IGNORE** – duplicate ATTENDANCE_id is skipped; no error, connection stays up.
- **ON DUPLICATE KEY UPDATE** – overwrite row on duplicate ATTENDANCE_id.
...
| `04-migrate-attendance-schema.sql` | One-time migration from old table (id INT, authDateTime) to new (ATTENDANCE_id, eventTime). **Run this before using the updated API** if your database still has the old schema. |

In iVMS-4200 Third-Party Database, map fields to:

- **ATTENDANCE_id** (e.g. Event ID / ID)
- **eventTime** (event time as DATETIME)
